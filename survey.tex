...
\begin{enumerate}
    ...
    \item \textit{Análise de abstract e metadados:} Os abstracts e informações de publicação foram analisados para verificar relevância e qualidade, resultando em 30 artigos pré-selecionados.
    \item \textit{Leitura completa e rastreamento de referências:} Os artigos pré-selecionados foram lidos integralmente, e suas referências foram rastreadas para identificar trabalhos adicionais relevantes. Após essa etapa, 26 artigos foram selecionados para compor a base bibliográfica final deste \textit{survey}.
\end{enumerate}

\textbf{Trabalhos selecionados.}
A lista final de 26 referências inclui trabalhos seminais e \textit{surveys} abrangentes em cada uma das cinco áreas principais:
\begin{itemize}
	\item \textit{Machine Unlearning:} Bourtoule et al.~\cite{bourtoule2021machine} introduzem, definem e conceituam acerca de \textit{Machine Unlearning}, sendo este o principal artigo usado no direcionamento da pesquisa para esta \textit{survey}.
    ...
    \item \textit{Differential Privacy:} McMahan et al.~\cite{mcmahan2018learning} aplicam privacidade diferencial a modelos de linguagem recorrentes; Dwork et al.~\cite{DP01} fornece conceitos gerais para o entendimento da differential privacy; ~\cite{DP02} aplicações práticas de differential privacy; ~\cite{DP03} fundamentação algorítma da differential privacy;~\cite{DP04} aprofunda o parâmetro delta e o conceito de falha de privacidade;~\cite{DP05} aprofundamento do tema de local differential privacy; ~\cite{DP06} foca nos riscos de privacidade em machine unlearning.
...
\textbf{Contexto.} Desaprendizado de Máquina (ou Machine Unlearning), é um ramo da Aprendizagem de Máquina com foco em realizar um processo de remoção de dados específicos de um modelo após este já ter sido treinado, sem precisar retreiná-lo do zero ~\cite{bourtoule2021machine}. 
Sua proposta surgiu a partir da necessidade de fazer um modelo de aprendizado de máquina ser capaz de remover determinados tipos de dados usados em seu treinamento, como aqueles com desinformação, conteúdo sensível, informação datada e entre outros, quando o artigo 17 do Regulamento Geral sobre a Proteção de Dados da União Europeia entrou em vigor em 2014~\cite{bourtoule2021machine}.
...
\textbf{Tipos.}
\begin{enumerate}
    \item\textbf{SISA:} Acrônimo para Shared, Isolated, Sliced, Aggregated, a estratégia SISA permite a remoção de dados específicos sem a necessidade de retreinar o modelo inteiro do zero, tornando, dessa forma, a desaprendizagem de máquina mais eficiente e menos custosa computacionalmente~\cite{bourtoule2021machine}.  A seguir, pode-se ver uma explicação breve de cada componente dessa estratégia:
    \begin{enumerate}
...
\end{enumerate}
\textbf{Conclusões.} \textit{Machine Unlearning} surge como uma resposta à necessidade de regulamentação da proteção de dados, especialmente na remoção de dados, imposta por legislações modernas. O Desaprendizado de Máquina oferece soluções que garantem determinadas vantagens, desvantagens e desafios.
\begin{enumerate}
    \item\textbf{Vantagens}
    \begin{enumerate}
        \item\textit{Eficiência Computacional:} Com técnicas eficientes de \textit{Machine Unlearning}, o custo computacional e o tempo total de atualização do modelos são reduzidos de maneira significativa em comparação com o simples re-treinamento do modelo do zero~\cite{bourtoule2021machine,golatkar2020eternal}.        
        \item\textit{Governança sobre os Dados:} Como \textit{Machine Unlearning} permite a remoção de dados re maneira ágil e sem gerar custos, isso garante políticas de gestão de dados, permitindo a alteração do modelo de maneira controlada e transparente.       
    \end{enumerate}

\item\textbf{Desvantagens}
    \begin{enumerate}
        \item\textit{Ausência de garantia:} Embora as abordagens do desaprendizado de máquina sejam práticas, elas não garantem que o resultado final do modelo seja exatamente igual ao que fosse caso tenha sido treinado do zero~\cite{DP06}. 
        Esse fato foi frequentemente postulado durante as explicações, nesta survey, das principais técnicas de \textit{Machine Unlearning}.
        
        \item\textit{Complexidade de implementação:} As técnicas de desaprendizagem exigem diversas modificações que são significativas no treinamento do modelo, sendo difícil de implementar nos modelos já existentes.
        
        \item\textit{Custo de Armazento:} As estratégias de \textit{Machine Unlearning} exigem armazenamento de múltiplos modelos e, possivelmente, várias cópias de dados diferentes, diferentemente de técnicas tradicionais, o que torna implementações de \textit{Machine Unlearning} dispendiosas em relação à memória.
    \end{enumerate}

\item\textbf{Desafios}
    \begin{enumerate}
        \item\textit{Equilíbrio entre privacidade e eficiência:} Os modelos possibilitam a remoção de dados, mas podem prejudicar a confiabilidade do modelo. Um dos principais desafios consiste nessa conciliação de privacidade de dados e a eficiência do modelo após alterações~\cite{bourtoule2021machine,DP06};
        
        \item\textit{Automação de Processo:} Por ainda ser recente, ainda se tem como desafia a criação de sistemas capazes de lidar com os pedidos de \textit{unlearning} de forma automatizada;
        
        \item\textit{Aplicação em cenários reais:} Ainda existem desafios na aplicação dos métodos em cenários reais, como em ambientes de aprendizado distribuído, sistemas de escala industrial, sistemas governamentais e entre outros.
    \end{enumerate}
\end{enumerate}
...
\end{equation}
\end{enumerate}
\textbf{Tipos.} Existem atualmente dois tipos amplamente utilizados de differential privacy, a DP centralizada e a DP local, que diferem entre si pelo momento onde o ruído é adicionado. ~\cite{DP01}
\begin{enumerate}
