...
\end{enumerate}
\textbf{Conclusões.} \textit{Machine Unlearning} surge como uma resposta à necessidade de regulamentação da proteção de dados, especialmente na remoção de dados, imposta por legislações modernas. o Desaprendizado de Máquina oferece soluções que garantem determinadas vantagens, desvantagens e desafios, os quais podemos ver a seguir:
\begin{enumerate}
    \item\textbf{Vantagens}
    \begin{enumerate}
        \item\textit{Eficiência Computacional:} Com \textit{Machine Unlearning}, o custo computacional e o tempo total de atualização do modelos são reduzidos de maneira significativa~\cite{bourtoule2021machine,golatkar2020eternal};
        
        \item\textit{Respostas ágeis:} Por serem estratégias que garantem a alteração do modelo sem ter que re-calcular desde o início, o desaprendizado de máquina possibilita a remoção de dados de maneira rápida caso seja solicitado~\cite{bourtoule2021machine};
...
\item\textbf{Desvantagens}
    \begin{enumerate}
        \item\textit{Ausência de garantia:} Embora as abordagens do desaprendizado de máquina sejam práticas, elas não garantem que o resultado final do modelo seja exatamente igual ao que fosse caso tenha sido treinado do zero, podendo manter elementos influenciados pelos dados removidos ~\cite{DP06};
        ...
        \item\textit{Custo de Armazento:} As estratégias de \textit{Machine Unlearning} exigem armazenamento de diversos modelos, grandes conjuntos de dados e entre outros, o que torna inviável em determinados ambientes.
    \end{enumerate}
...
\item\textbf{Desafios}
    \begin{enumerate}
        ...
        \item\textit{Aplicação em cenários reais:} Assim como no item anterior, o fato de \textit{Machine Unlearning} ser recente, ainda existem desafios na aplicação dos métodos em cenários reais, como em ambientes de aprendizado distribuído, sistemas de escala industrial, sistemas governamentais e entre outros.
    \end{enumerate}
\end{enumerate}
